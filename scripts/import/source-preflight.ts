...
export function requireApprovedSource(source: "era5Land" | "copernicusDem") {
  ...
  if (source === "era5Land") {
    ...
    if (value.temperatureUnitExpected!=="K"||value.windUnitExpected!=="m s-1"||value.precipitationUnitExpected!=="m"||value.snowDepthUnitExpected!=="m"||value.snowCoverSourceUnitExpected!=="%"||value.snowCoverNormalization!=="PERCENT_TO_FRACTION"||value.snowCoverSemantics!=="FRACTION_0_TO_1") throw new Error("BLOCKED_SOURCE_SEMANTICS: ERA5-Land units or snow semantics are not fully approved.");

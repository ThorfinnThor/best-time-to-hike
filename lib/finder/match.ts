...
export function matchDestinations(destinations: SearchDestination[], preferences: FinderPreferences) {
  return destinations
    .filter((destination) => preferences.region === "all" || destination.continent === preferences.region || destination.region === preferences.region)
    .map((destination) => {
      const month = destination.monthly.find((item) => item.m === preferences.month)!;

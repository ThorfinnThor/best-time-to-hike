...
export function Finder({destinations,locale,compact=false}:{destinations:SearchDestination[];locale:Locale;compact?:boolean}) {
  const [month,setMonth]=useState(5); const [region,setRegion]=useState("all"); const [minTemp,setMinTemp]=useState(10); const [maxTemp,setMaxTemp]=useState(24); const [avoidRain,setAvoidRain]=useState(true); const [avoidSnow,setAvoidSnow]=useState(true);
  ...
  return <section className={`finder ${compact?"finder-compact":""}`} aria-label={locale === "de" ? "Wanderziel-Finder" : "Hiking destination finder"}>
    ...
    <div className="finder-results">{matches.map(({destination,month:result,match},index)=><Link key={destination.slug} href={destinationPath(locale,destination.slug,month)} className="result-card"><span className="result-rank">0{index+1}</span><div><strong>{destination.name}</strong><small>{destination.countryCode} · {Math.round(result.temp)}°C · {Math.round(result.wet*100)}% {locale === "de" ? "Regentage" : "wet days"}</small></div><div className="result-score"><strong>{match}%</strong><span>{locale === "de" ? "Match" : "match"}</span></div></Link>)}</div>
    {compact&&<Link className="text-link" href={`/${locale}/finder`}>{locale === "de" ? "Alle Ergebnisse ansehen →" : "Explore all results →"}</Link>}
  </section>;
}

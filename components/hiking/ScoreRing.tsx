import type { Locale, ScoreLevel } from "@/lib/data/types";
import { t } from "@/lib/i18n/dict";
import { scoreLevel } from "@/lib/scoring/index";

// The ring colour follows the published score ladder. It used to carry its own
// 85/65 boundaries, which were the *confidence* thresholds applied to a score,
// so a "very-good" 82 was drawn in the middling colour (mistakes.md #13).
const RING_COLOR = {
  excellent: "#416f52",
  "very-good": "#416f52",
  good: "#c38b3d",
  fair: "#c65f42",
  poor: "#c65f42",
} as const;

// `level` is the published label where the caller has one, so a month whose
// label is capped is not drawn in the colour of the score it did not keep.
export function ScoreRing({score,level,size="large",locale}:{score:number;level?:ScoreLevel;size?:"large"|"small";locale:Locale}) {
  const color = RING_COLOR[level ?? scoreLevel(score)];
  return <div className={`score-ring ${size}`} role="img" style={{background:`conic-gradient(${color} ${score*3.6}deg, rgba(255,255,255,.22) 0deg)`}} aria-label={t(locale).common.scoreOutOf(score)}><span><strong>{score}</strong><small>/100</small></span></div>;
}

...
import batch from "../data-config/sources/destination-candidates-batch-5.json";
...
test("batch five contains exactly forty distinct planning-only destinations", () => {
  assert.equal(batch.status, "planning-only");
  assert.equal(batch.candidates.length, 40);
  assert.equal(new Set(batch.candidates.map((candidate) => candidate.id)).size, 40);
  assert.equal(new Set(batch.candidates.map((candidate) => candidate.continent)).size, 6);
});

test("batch five coordinates, time zones and taxonomy are publishable", () => {
...
test("batch five does not alias another destination model cell", () => {

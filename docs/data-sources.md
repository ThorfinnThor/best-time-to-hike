...
`scripts/import/download_era5_land_orography.py` selects the nearest invariant grid coordinate for every unique point in the ERA5 request plan, requires parameter `z` in `m**2 s**-2`, and converts geopotential to model elevation using `elevation_m = z / 9.80665`. Candidate staging first resolves every DEM candidate against this field and excludes points more than 600 m from the target band before any 30-year climate request. It records requested and resolved coordinates, raw geopotential, converted height, pinned source metadata and retrieval time in the staging evidence. The climate importer also verifies that the invariant and time-series downloads resolve to the same grid coordinate before aggregation. The Python importer records the ECMWF glacier-indicator count for snow depth values at or above 10 m; candidate staging rejects such cells when the destination scope excludes glacier terrain.
